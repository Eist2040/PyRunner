...
class CoreConfig(AppConfig):
    ...
    def _register_worker_signals(self):
        ...
        try:
            ...
            HEARTBEAT_THROTTLE_SECONDS = 20

            def update_heartbeat(sender, task, **kwargs):
                """
                Update heartbeat timestamp after each task execution.

                Throttled: with 1-minute schedules across many scripts this
                signal can fire dozens of times a minute. Writing the DB
                row every time is pure overhead — the heartbeat only needs
                minute-level resolution, so skip if we wrote recently.
                """
                if not cache.add(
                    "pyrunner:heartbeat_throttle", 1, HEARTBEAT_THROTTLE_SECONDS
                ):
                    return
                try:
