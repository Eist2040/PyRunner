...
@login_required
def script_export_view(request: HttpRequest, pk) -> HttpResponse:
    """Download a script's code as a standalone .py file."""
    script = get_object_or_404(Script, pk=pk)
    filename = f"{slugify(script.name) or 'script'}.py"
    resp = HttpResponse(script.code, content_type="text/x-python; charset=utf-8")
