...
class ScriptForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "memory_limit_mb": forms.NumberInput(
                attrs={
                    "class": "w-full px-4 py-3 bg-code-bg border border-code-border rounded-lg text-code-text focus:outline-none focus:ring-2 focus:ring-code-accent/50 focus:border-code-accent",
                    "min": 0,
                }
        ...
        help_texts = {
            "timeout_seconds": "Maximum execution time (1 second to 72 hours)",
            "memory_limit_mb": "Hard RAM cap for this script's process (0 = unlimited). Scripts at/above the heavy-job threshold (Settings) are queued to run one-at-a-time regardless of worker count.",
    ...
    def clean_timeout_seconds(self):
        ...
        if timeout is not None and (timeout < 1 or timeout > 259200):
            raise forms.ValidationError("Timeout must be between 1 and 259200 seconds (72 hours).")

    def clean_memory_limit_mb(self):
        limit = self.cleaned_data.get("memory_limit_mb")
        if limit is not None and limit != 0 and limit < 64:
            raise forms.ValidationError("Memory limit must be 0 (unlimited) or at least 64 MB.")
        return limit
        return timeout
...
class WorkerSettingsForm(forms.Form):
    ...
    q_workers = forms.IntegerField(
        ...
        label="Worker Count",
        help_text="Number of worker processes (1-50). More workers can process more tasks simultaneously.",
    )
    ...
    # GOAT STRATEGY: independent of q_workers above. Lets you run 3-4
    # workers for throughput while capping how many memory-heavy scripts
    # (see Script.memory_limit_mb) execute at the same instant.
    heavy_job_threshold_mb = forms.IntegerField(
        min_value=0,
        max_value=1_000_000,
        initial=512,
        widget=forms.NumberInput(
        ...
        label="Heavy Job Threshold (MB)",
        help_text="Scripts with a memory limit at/above this are gated by Heavy Job Slots below, regardless of worker count.",
    )

    heavy_job_slots = forms.IntegerField(
        min_value=1,
        max_value=50,
        initial=1,
        widget=forms.NumberInput(
        ...
        label="Heavy Job Slots",
        help_text="Max heavy (memory-intensive) scripts allowed to run at the same time. Keep low to protect RAM even with 3-4 workers.",
    )
    ...
    def __init__(self, *args, instance=None, **kwargs):
        ...
        if instance:
            ...
            self.fields["heavy_job_threshold_mb"].initial = instance.heavy_job_threshold_mb
            self.fields["heavy_job_slots"].initial = instance.heavy_job_slots
    ...
    def save(self, instance):
        ...
        instance.heavy_job_threshold_mb = self.cleaned_data["heavy_job_threshold_mb"]
        instance.heavy_job_slots = self.cleaned_data["heavy_job_slots"]
