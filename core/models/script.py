...
class Script(models.Model):
    ...
    # GOAT STRATEGY: worker count controls THROUGHPUT (how many scripts can
    # run at once), memory_limit_mb controls RAM (how much any ONE of them
    # can use). These are independent — raising workers to 3-4 for
    # parallelism is safe as long as every subprocess is hard-capped here.
    # Enforced via RLIMIT_AS in executor.py (OS kills the process itself if
    # it crosses the line — no app-level polling, zero overhead).
    memory_limit_mb = models.PositiveIntegerField(
        default=512,
        help_text="Hard RAM cap for this script's process, in MB (0 = unlimited). "
