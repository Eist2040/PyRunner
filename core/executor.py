...
# --- GOAT STRATEGY: per-job RAM cap + heavy-job concurrency gate --------
# These two together let q_workers stay at 3-4 for throughput without
# workers determining RAM usage. Cap comes from Script.memory_limit_mb,
# gate comes from GlobalSettings.heavy_job_slots. Neither touches the
# worker process itself (that's Q_CLUSTER's max_rss, a separate knob).

def _make_memory_limit_preexec_fn(memory_limit_mb: int):
    """
    Return a preexec_fn that sets an RLIMIT_AS (virtual memory) cap on the
    child process before exec. Runs in the forked child, before the script
    starts — the kernel enforces it, so this is zero-overhead compared to
    polling RSS from the parent.

    0 or None => no cap (falls through to just setsid for process-group
    control, same as before this feature existed).
    """
    import resource
    ...
    def _preexec():
        ...
        if memory_limit_mb and memory_limit_mb > 0:
            ...
            try:
                ...
                # Some platforms/containers don't allow RLIMIT_AS — fail
                # open rather than crash the run over an unrelated issue.
                pass

    return _preexec


def _acquire_heavy_slot(run: Run) -> bool:
    """
    Block (with polling) until a heavy-job slot is free, or the script's
    own timeout elapses. Returns True if acquired, False on timeout.

    Only called for scripts whose memory_limit_mb >= heavy_job_threshold_mb.
    Uses select_for_update on the GlobalSettings singleton row as a simple
    cross-process counting semaphore — no Redis/extra infra needed, and
    django-q2 already requires the DB to be reachable for the ORM broker.
    """
    import time
    from django.db import transaction
    from core.models import GlobalSettings

    # ROOT FIX: don't block for the script's full timeout_seconds (could be
    # 30-60+ min for heavy scripts like bulk_seo_articles). That left a
    # queued Run silently squatting for an hour before its subprocess ever
    # started - never claiming its WP job row, never calling back, so the
    # WP-side job looked permanently stuck 'pending' with zero feedback.
    # Cap the wait so a starved Run fails fast with the existing clean
    # "timed out waiting for a free heavy-job slot" message instead, and
    # let the caller (WP rescue/gate-dispatch cron) retry on its own cadence.
    deadline = time.monotonic() + min(max(run.script.timeout_seconds, 30), 120)
    while time.monotonic() < deadline:
        with transaction.atomic():
            settings_row = GlobalSettings.objects.select_for_update().get(pk=1)
            if settings_row.heavy_job_running_count < max(settings_row.heavy_job_slots, 1):
                settings_row.heavy_job_running_count += 1
                settings_row.save(update_fields=["heavy_job_running_count"])
                return True
        time.sleep(2)
    logger.warning(f"Run {run.id} timed out waiting for a heavy-job slot")
    return False


def _release_heavy_slot() -> None:
    """Release a previously-acquired heavy-job slot. Never raises."""
    from django.db import transaction
    from core.models import GlobalSettings
    ...
    try:
        with transaction.atomic():
            settings_row = GlobalSettings.objects.select_for_update().get(pk=1)
            if settings_row.heavy_job_running_count > 0:
                settings_row.heavy_job_running_count -= 1
                settings_row.save(update_fields=["heavy_job_running_count"])
    except Exception:
        logger.exception("Failed to release heavy-job slot")


def _is_heavy(run: Run) -> bool:
    from core.models import GlobalSettings

    settings_row = GlobalSettings.get_settings()
    return run.script.memory_limit_mb >= settings_row.heavy_job_threshold_mb > 0
...
def execute_run(run: Run, webhook_data: dict | None = None) -> None:
    ...
    try:
        ...
        # Phase 2.5: Heavy-job gate — if this script's memory cap qualifies
        # it as "heavy", wait for a free slot before consuming a worker on
        # actual execution. Cheap/small scripts skip this entirely and
        # just run (bounded only by the normal worker pool).
        heavy_slot_held = False
        if _is_heavy(run):
            if not _acquire_heavy_slot(run):
                run.status = Run.Status.FAILED
                run.stderr = (
                    "[Run failed: timed out waiting for a free heavy-job slot. "
                    "Too many memory-intensive scripts running concurrently — "
                    "raise GlobalSettings.heavy_job_slots or stagger schedules.]"
                )
                run.ended_at = timezone.now()
                run.save()
                logger.warning(f"Run {run.id} failed - no heavy-job slot available")
                return
            heavy_slot_held = True
        ...
        try:
            ...
            # reliably kills the whole child tree (Unix), AND cap its
            # virtual memory via RLIMIT_AS so one runaway script can't eat
            # the whole box regardless of worker count. On Windows, hide
            # the console window and use a new process group (RLIMIT_AS
            # has no Windows equivalent — memory_limit_mb is a no-op there).
            if os.name == "posix":
                popen_kwargs["preexec_fn"] = _make_memory_limit_preexec_fn(
                    run.script.memory_limit_mb
                )
            else:
