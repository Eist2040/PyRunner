...
def _get_q_cluster_config():
    ...
    # ROOT NOTE: 2 was too low for a multi-tool platform — any 2 running
    # scripts (of ANY kind: webhook dispatch, scheduled poll, manual run)
    # exhausted the entire pool platform-wide, queueing everything else
    # behind them regardless of per-script concurrency settings. Existing
    # installs: this fallback only applies before the global_settings DB row
    # exists — use `python manage.py scale_workers N` to change a live value.
    config = {
        "name": "PyRunner",
        ...
        "orm": "default",
        "catch_up": False,
        # RAM FIX: recycle a worker after N tasks so per-process leaks
        # (secrets decryption, subprocess env copies, DB connections)
        # don't accumulate forever. Critical for 1-minute schedules —
        # without this a worker can run thousands of tasks/day and never
        # release memory back to the OS.
        "recycle": int(os.environ.get("Q_RECYCLE", 50)),
        # RAM FIX: hard memory ceiling per worker (KB). django-q2 kills
        # and respawns a worker that exceeds this, independent of task
        # count. 250MB is generous for typical scripts; raise via env
        # var if legitimate workloads need more.
        "max_rss": int(os.environ.get("Q_MAX_RSS", 250_000)),
        # Cap how many finished task results (Success/Failure rows) are
        # kept — unbounded growth here bloats the DB and slows every
        # queue-status query the whole platform makes.
        "save_limit": int(os.environ.get("Q_SAVE_LIMIT", 100)),
    }
